...
public class ReqFuncionalesModificacionTest {
...
//			e.printStackTrace();
		...
		try{	
			...
			//Solicitar un Carrito de compras.
			//IdCliente Ya está registrado en la tienda
			//idCarrito se conoce en la tienda, se obtiene a través de la interfaz
			// ( idCarrito, idCliente)
			//------------------------------------------------------------------------------------------
			Carrito asignado = superAndes.tomarCarrito(424, 1013254321);
			assertNotNull(asignado);
			//------------------------------------------------------------------------------------------

			
			//Requerimiento funcional 13
			//Adicionar un producto al carrito de compras.
			//El carrito ya ha sido tomado, el producto existe
			//(idCarrito, idProducto, cantidadProductos)
			// El carrito pertenece a la sucursal 22222
			//------------------------------------------------------------------------------------------
			superAndes.adicionarProductoACarrito(424, 1, 2);
			superAndes.adicionarProductoACarrito(424, 2, 2);
			List<Object[]> productosCarrito =superAndes.listarProductosDeCarrito(424);
			assertEquals(productosCarrito.size(), 2);
			//------------------------------------------------------------------------------------------
			...
			//Pagar la compra de un carrito
			//El carrito existe, por lo tanto el id está registrado.
			//El identificador de la sucursal existe.
			//El identificador del cliente existe y tomó el carrito.
			//(idCarrito, idSucursal, idCliente)
			//------------------------------------------------------------------------------------------
    	    int cantidaProductosFacturados = superAndes.facturarCarrito(461, 22222, 1014370672);
    	    assertEquals(1, cantidaProductosFacturados);
			//------------------------------------------------------------------------------------------
    	    
    	    
    	    //Requerimietno Funcional 16
    	    //Abandonar un carrito de compras
    	    //El idCarrito existe.
    	    //( idCarrito, estadoCarrito)
			//------------------------------------------------------------------------------------------
    	    int antes= superAndes.listarCarritosDisponibles(22222).size();
    	    superAndes.cambiarEstadoCarrito(424, "ABANDONADO");
    	    int despues= superAndes.listarCarritosDisponibles(22222).size();
    	    ...
    	    //El idCarro es válido, y tenía productos en él.
    	    //(idCarro)
			//------------------------------------------------------------------------------------------
    	    superAndes.recolectarProductosDeCarrito(424);
    	    int cantidadProductosCarroAbandonado = superAndes.listarProductosDeCarrito(424).size();
    	    assertEquals(cantidadProductosCarroAbandonado, 0);
			//------------------------------------------------------------------------------------------
